...
- **Phase** — 2 — Index layer and queries (not yet planned)
- **Focus** — Phase 2 plan
- **Blocked/Broken** — None
...
- **Phase 2 (next)** — Index layer and queries: `get_index`, `query_notes`, inbound link counting. Also retrofits Phase 1 `get_note` and `update_note` to read via the index instead of scanning tier directories (per D-8).
