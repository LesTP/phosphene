...
- **Distillation ↔ toolkit/clustering RAPTOR strategy** — the RAPTOR recursive clustering interface (cluster → summarize → re-cluster) requires the consumer to provide a summarizer callback. How distillation wires this to toolkit/llm_client is provisional. Resolve when writing ARCH_distillation.md.
