...
## Purpose
Adapters that pull content from external sources and normalize it into `ContentItem` objects for the Attention Filter. Each adapter is independent (leaf) with no cross-dependencies. Adapters cover both autonomous sources (channels the system monitors on its own) and human-curated sources (content the human explicitly shares). The adapter interface is uniform — the Attention Filter doesn't care where content came from, only the `source` field distinguishes origin.
