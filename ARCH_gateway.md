...
## Purpose
Multi-platform message bus for both inbound and outbound communication. Wraps toolkit/telegram_client (and future platform adapters) behind a uniform interface. Inbound: receives human messages and platform signals (reactions, replies), dispatches to the Orchestrator. Outbound: delivers generated content to the appropriate platform in the correct format. Separates Phosphene's messaging concerns from Source Ingestion's content-pulling concerns — even when both use Telegram, they serve different purposes.
